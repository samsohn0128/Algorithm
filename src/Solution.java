...
public class Solution {
    ...
    private static int solution() {
        for (int i = 0; i <= peopleList.size(); i++) {
            ...
            System.out.println("moveTimes[" + i + "][" + (peopleList.size() - i) + "] = " + moveTimes[i][peopleList.size() - i]);
        }
    ...
    private static void putAllPeopleInStepsPq(int numberOfPeopleInSteps1) {
        ...
        stepsPq2.addAll(peopleList.subList(numberOfPeopleInSteps1, peopleList.size()).stream()
                ...
                .collect(Collectors.toList()));
        stepsPq1.forEach(System.out::println);
        System.out.println();
    }
    ...
    private static int calculateMoveTime(PriorityQueue<Person> stepsPq, int stepsLength) {
        ...
        int currentTime = 0;
        while (!stepsPq.isEmpty()) {
            if (movingQueue.isEmpty()) {
                if (stepsPq.size() >= 3) {
                    stepsPq.poll();
                    stepsPq.poll();
                    currentTime = stepsPq.poll().arrivalTime + 1 + stepsLength;
                } else {
                    while (stepsPq.size() > 1) {
                        stepsPq.poll();
                    }
                    return stepsPq.poll().arrivalTime + 1 + stepsLength;
                }
            }
            while (!stepsPq.isEmpty() && stepsPq.peek().arrivalTime + 1 <= currentTime && movingQueue.size() < 3) {
                Person person = stepsPq.poll();
                person.waitingTime = currentTime - (person.arrivalTime + 1);
                movingQueue.offer(person);
            }
            if (!movingQueue.isEmpty()) {
                Person person = movingQueue.poll();
                currentTime = person.arrivalTime + 1 + person.waitingTime + stepsLength;
            }
        }
        return currentTime;
    }
